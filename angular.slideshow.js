  ...
  .directive('slideshowContainer', function() {
    return {
      link: function(scope, element, attr) {
        ...
        angular.extend(touchSlider.prototype, {
          ...
          findEl: function() {
            ...
            this.panel = this.panel && container.find(this.panel) || this.wrap.children().eq(0);
          ...
          init: function() {
            ...
            this.steps = this.steps || parseInt(window.getComputedStyle(this.wrap[0]).width);
            angular.forEach(panels, function(item, n) {
          ...
          getImg: function(obj) { //加载图片
            ...
            var that = this,
              ...
              cls = 'img' + lazy;
            lazy = lazy.replace(/^\.|#/g, '');
            angular.forEach(obj.find(cls), function(item, n) {
              var nobj = angular.element(item);
          ...
          destroy: function() {
            ...
            if (that.hasTrigger && triggers) {
              triggers.each(function(n, item) {
                angular.element(item).off('click');
              });
            }
          }
        });
